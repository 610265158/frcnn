...
def Random_scale_withbbox(image,bboxes,target_shape,jitter=0.5):
    ...
    while 1:
        ...
        bboxes_[:, 0] = bboxes_[:,0]-start_h
        bboxes_[:, 1] = bboxes_[:, 1] - start_w
        bboxes_[:, 2] = bboxes_[:, 2] - start_h
        bboxes_[:, 3] = bboxes_[:, 3] - start_w
    ...
    rescale_h=int(h * random.uniform(0.2, 1))

    rescale_w = int(rescale_h/(random.uniform(0.2, 1.8)*croped_h_w_ratio))
    ...
    bboxes_fix[:, 0] = bboxes_fix[:,0]*rescale_h/croped_h+dy
    bboxes_fix[:, 1] = bboxes_fix[:, 1] * rescale_w / croped_w+dx
    bboxes_fix[:, 2] = bboxes_fix[:, 2] * rescale_h / croped_h+dy
    bboxes_fix[:, 3] = bboxes_fix[:, 3] * rescale_w / croped_w+dx
