...
def fastrcnn_outputs(scope_str,feature, num_classes,L2_reg,training=True,class_agnostic_regression=False):
...
def fastrcnn_2fc_head_(scope_str,feature,L2_reg,training):
    """
    Args:
        feature (any shape):
    ...
    with slim.arg_scope(arg_scope):
        with slim.arg_scope([slim.batch_norm], is_training=training):
            with tf.variable_scope(scope_str):
                hidden = slim.fully_connected(feature, dim, activation_fn=tf.nn.relu, scope='fc6')
                hidden = slim.fully_connected(hidden, dim, activation_fn=tf.nn.relu, scope='fc7')
