...
def create_global_net(blocks, L2_reg,is_training, trainable=True):
    global_fms = []
...
def plain_resnet50_backbone(image,L2_reg,is_training=True):

    resnet_fms = resnet50(image, L2_reg,is_training, bn_trainable=True)
    print('resnet50 backbone output:',resnet_fms)

    # with tf.variable_scope('CPN'):
    fpn_fms = create_global_net(resnet_fms, L2_reg,is_training)
